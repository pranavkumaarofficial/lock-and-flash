...
## Design Requirements - Material 3 Expressive (2026)

READ M3_EXPRESSIVE_GUIDELINES.md for complete design specifications.

Key requirements:
- Use rounded containers (24dp radius) for all cards
- Bold primary colors (vibrant purple/blue)
- Spring animations (not linear)
- Larger buttons (56dp minimum height)
- Tonal elevation (surface color variants)
- Google Pixel aesthetic (playful, colorful, premium)

The app should look like it belongs on a Pixel 9 Pro running Android 16.
```

### **Step 3: Prompt for Claude Code Agent**

Open Claude Code and use this prompt:
```
I need to redesign the ScreenTracker app UI to match Material 3 Expressive (2026 Pixel style).

IMPORTANT: Read M3_EXPRESSIVE_GUIDELINES.md first for design specifications.

Current issues:
1. UI looks generic/AI-generated
2. Cards are too flat
3. Colors are muted
4. Animations are basic
5. Not enough visual hierarchy

Transform the UI to be:
- Premium Google Pixel aesthetic
- Bold, vibrant colors (purple-forward)
- Rounded containers (24dp radius)
- Spring-based animations
- Larger, more prominent buttons
- Better visual hierarchy with size variation
- Tonal surface colors (not flat white)

Focus on MainActivity.kt. Make it look like a first-party Google app.

Also add: A "Clear All Data" button in the UI to delete all events from the database.

Start by analyzing the current UI code and proposing specific M3 Expressive improvements.